        ...
        <form method="POST" action="{{ route("admin.items.update", [$item->id]) }}" enctype="multipart/form-data">
           ...
           <div class="row"> 
            ...
            <div class="form-group col-sm-4">
                ...
                <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $item->quantity) }}" step="1" required>
              ...
              <div class="form-group col-sm-4">
                ...
                @foreach(App\Item::ITEM_CATEGORY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('item_category') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="item_category_{{ $key }}" name="item_category" value="{{ $key }}" {{ old('item_category', $item->item_category) === (string) $key ? 'checked' : '' }}>
                        <label class="form-check-label" for="item_category_{{ $key }}">{{ $label }}</label>
                    </div>
                @endforeach
