        ...
        <form method="POST" action="{{ route("admin.accounts.update", [$account->id]) }}" enctype="multipart/form-data">
           ...
           <div class="row"> 
            ...
            <div class="form-group col-sm-4">
                ...
                <input class="form-control {{ $errors->has('designation') ? 'is-invalid' : '' }}" type="text" name="designation" id="designation" value="{{ old('designation', $account->designation) }}" required>
            ...
            <div class="form-group col-sm-4">
                <label class="required" for="departments">{{ trans('cruds.account.fields.department') }}</label>
                <select class="form-control {{ $errors->has('departments') ? 'is-invalid' : '' }}" name="departments[]" id="departments" required>
                    @foreach($departments as $id => $department)
                        <option value="{{ $id }}" {{ (in_array($id, old('departments', [])) || $account->departments->contains($id)) ? 'selected' : '' }}>{{ $department }}</option>
                    @endforeach
                </select>
