...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group col-sm-4">
                ...
                @foreach(App\User::GENDER_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('gender') ? 'is-invalid' : '' }}">
                        ...
                        <label style="float;right" class="form-check-label" for="gender_{{ $key }}">{{ $label }}</label>
                    </div>
                @endforeach
                ...
                <div class="form-group col-sm-4">
                    ...
                    @foreach(App\User::POSITION_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('position') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="position_{{ $key }}" name="position" value="{{ $key }}" {{ old('position', '') === (string) $key ? 'checked' : '' }}>
                            <label style="float;right" class="form-check-label" for="position_{{ $key }}">{{ $label }}</label>
                        </div>
                    @endforeach
                    @if($errors->has('gender'))
                        <div class="invalid-feedback">
          ...
          <div class="form-group col-sm-4">
            <label class="required" for="password">{{ trans('cruds.user.fields.password_confirmation') }}</label>
            <input class="form-control {{ $errors->has('password') ? 'is-invalid' : '' }}" type="password" name="password_confirmation" id="password_confirmation" required>
            
            <span class="help-block">{{ trans('cruds.user.fields.confirmation_helper') }}</span>
        </div>
      </div>
